...
// Define Internship struct (used only here)
struct Internship {
    string jobID;
    string jobTitle;
    string company;
...
// Helper function to split a line using '|' delimiter
...
// Load internship listings from job_listings.txt
...
// Save a new application to applications.txt
...
int main() {
    ...
    for (size_t i = 0; i < internships.size(); ++i) {
        cout << i + 1 << ". " << internships[i].jobTitle
             ...
             << " (Deadline: " << internships[i].deadline << ")" << endl;
    }
